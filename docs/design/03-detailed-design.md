...
#### 1.1.1 なぜ主キーが `bigint` なのか

> 主キーの型は、二次索引の大きさと公開URLの安全性の両方を決める。

**`int` ではない理由。** 符号付き `int` の上限は約21億。`auto_increment` は
**削除された番号を再利用しない**ため、行の出入りが多い表は思ったより早く上限に
近づく。問題は上限そのものより **後から直す費用** で、
`ALTER TABLE ... MODIFY id bigint` は大きな表では全体を作り直して長時間ロックする。
1行あたり4バイト増えることを、その停止時間と引き換えにする理由がない。

**UUID ではない理由。** InnoDB はクラスタ索引で、**主キーがクラスタキーそのもの**。
さらに **すべての二次索引の葉に主キー値が行ポインタとして入る**。

| | `bigint` | `UUID` / `CHAR(36)` |
| --- | --- | --- |
| 主キー幅 | 8 バイト | 36 バイト |
| **二次索引の付帯コスト** | 1件あたり 8 バイト | 1件あたり **36 バイト** |
| 挿入位置 | 常に最右ページへ追記 | 木全体へランダムに分散 |
| ページ分割・断片化 | ほぼ起きない | 頻発 |
| 比較 | 整数比較 | 文字列比較 |

`album_photo` には `uk_albumId_photoId` / `idx_albumId_position` / `idx_photoId`
の3索引がある。主キーが36バイトになると、この3つがまとめて3〜4倍に膨らむ。
索引が大きいほどバッファプールに載るページが減り、命中率が下がる。

加えて UUIDv4 は**ランダム**なので挿入点が毎回変わり、ページ分割と断片化を
生み続ける。`auto_increment` は常に同じ最右ページへ追記するため、書き込みが
一点に集中してキャッシュに乗る。

#### 1.1.2 `bigint` の代償 — 連番は数えられる

> 自増IDは推測できる。この事実から2つの制約が導かれる。

`/api/albums/56` が存在するなら `55` も存在する。1から順に叩けば全件を
列挙できてしまう。これは `bigint` を選んだ以上、避けられない性質である。

**そこから導かれる制約は2つ。どちらも省略できない。**

| # | 制約 | 理由 |
| --- | --- | --- |
| 1 | **認証不要の入口では、IDではなく乱数トークンを使う** | `/s/{token}` は未ログインで開ける。ここにIDを置けば全ユーザーのアルバムが総当たりで読める。`album_share.shareToken` が `SecureRandom` 24バイト → base64url 32文字なのはこのため（§5.5） |
| 2 | **認証ありのAPIは、全エンドポイントで所有権を検証する** | IDが推測可能なので、検証を1か所でも忘れると IDOR になる。他人の `albumId` を直接叩けば読めてしまう |

制約2の実装は `album_member` の存在確認で、違反は 403 `ALBUM_FORBIDDEN`（§3.2）。
**これは書いておくだけでは足りない項目で、§7 のテスト計画に「他人の `albumId`
直叩きで403」を結合テストとして入れてある。必ず実際に叩いて確認すること。**

副次的な性質として、IDは事業規模を漏らす（アカウントを2つ作ってIDの差を見れば、
その間の登録数が分かる）。ハッカソンでは許容する。
